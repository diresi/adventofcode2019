...
def visit(g, visitor, start="COM", dfs=False):
    todo = deque([start])

    # it's a DAG, I know. but with a little addon we can handle cyclic graphs
    seen = set()
    while todo:
        n = todo.popleft()

        # 3 lines to handle cyclic graphs :)
        if n in seen:
            continue
        seen.add(n)
        ...
        if dfs:
            todo.extendleft(next_nodes)
        else: # bfs
            todo.extend(next_nodes)
